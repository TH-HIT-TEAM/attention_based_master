...
def filter(r_path, w_path):
    ...
    rf = []
    for d in data:
        sent_blind, relation, word_path_string, deprel_path_string, pos_path_string, tree_line, tree_pos_line, sent_id, sent, entities = d.strip().split('\n')
        ra.append(relation)
        ...
        if not r1 and not r2 and not r3 and not r4 and not r5 and not r6 and not r7 and not r8 and not r9 and e1!=e2:
            ...
            of.write(sent_blind + '\n')
            of.write(relation + '\n')
            of.write(word_path_string + '\n')
            of.write(deprel_path_string + '\n')
            of.write(pos_path_string + '\n')
            of.write(tree_line + '\n')
            of.write(tree_pos_line + '\n')
            of.write(sent_id + '\n')
            of.write(sent + '\n')
            of.write(entities + '\n')
            of.write('\n')
...
# execute
filter("./ddi_corpus/04dependencydfs/train_data.txt", "./ddi_corpus/05negativefilt/train_data.txt")
filter("./ddi_corpus/04dependencydfs/test_data.txt", "./ddi_corpus/05negativefilt/test_data.txt")
